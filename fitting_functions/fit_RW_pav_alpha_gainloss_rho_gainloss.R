fit_RW_pav_alpha_gainloss_rho_gainloss<-function(data, 
                                                 alphaBound, 
  ...
  X0<-c( runif(1),  runif(1), 
      runif(1, min = rhogainBound[1], max = rhogainBound[2]), 
      runif(1, min = rholossBound[1], max = rholossBound[2]))  
  
  #  distributon with mean 1
  LB<-c(alphagainBound[1],alphalossBound[1], rhogainBound[1], rholossBound[1]) # lower boundary
  UB<-c(alphagainBound[2], alphalossBound[2], rhogainBound[2], rholossBound[2]) 

  
  obfunc<-function(x) likelihood_RW_pav_alpha_gainloss_rho_gainloss(Data = data, 
                                                       alphagain = x[1] ,
                                                       alphaloss = x[2],
                                                       rhogain = x[3], 
                                                       rholoss = x[4],
  ...
  data <- list("alphagain" = NegLL[[1]][1], "alphaloss" = NegLL[[1]][2],
               'rhogain' = NegLL[[1]][3],'rholoss' = NegLL[[1]][4],
               "logLikel" = LL,"BIC"= BIC)
