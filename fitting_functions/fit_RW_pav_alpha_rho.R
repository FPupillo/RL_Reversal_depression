fit_RW_pav_alpha_rho<-function(data, 
                            alphaBound, 
  ...
  X0<-c( runif(1),  runif(1, min = rhoBound[1], max = rhoBound[2]))

  LB<-c(alphaBound[1],rhoBound[1]) # lower boundary
  UB<-c(alphaBound[2], rhoBound[2]) 

  
  obfunc<-function(x) likelihood_RW_pav_alpha_rho(Data = data, 
                                                       alpha = x[1] ,
                                                       rho = x[2], 
